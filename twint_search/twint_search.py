import twint
# from twint_search.terms import list_of_terms

list_of_terms = [
    "antibody test",
    "apron",
    "atriumhealthproud",
    "battleruncovid19",
    "besafe",
    "canceleverything",
    "cdc",
    "ce / fda",
    "ce / fda certified",
    "centre",
    "for disease control",
    "cheer4healthworkers",
    "china",
    "china virus",
    "chinavirus",
    "chinese virus",
    "chinesevirus",
    "chloroquine",
    "clean high touch surfaces regularly",
    "cloth mask",
    "convid19uk",
    "corona",
    "corona virus",
    "coronakindness",
    "coronapocalypse",
    "coronavirus",
    "coronavirus drug",
    "coronavirus vaccine",
    "coronaviruscrisis",
    "coronavirusdeutschland",
    "coronavirusimpact",
    "coronavirusinsouthafrica",
    "coronavirusisreal",
    "coronavirusoutbreakindia",
    "coronawarriors",
    "coronials",
    "covd",
    "covid",
    "covid tests",
    "covid - 19",
    "covid - 19 carrier",
    "covid - patient",
    "covid19",
    "covid19 diagnosis",
    "covid19 epidemic",
    "covid19 nucleic acid test",
    "covid19 pandemic",
    "covid19ireland",
    "covid19malaysia",
    "covid19ph",
    "covid19singapore",
    "covid19uk",
    "covid19update",
    "covid_19",
    "covidcarezw",
    "covidcoping",
    "covidheroes",
    "covidiot",
    "covidiots",
    "covidlife",
    "covidresearch",
    "covidwarriors",
    "covid - 19",
    "decontamination",
    "difficulty breathing",
    "disinfectant",
    "disinfection services",
    "disinfection team",
    "dontbeaspreader",
    "don't touch your face",
    "dr.fraud fauci",
    "duringmy14dayquarantine",
    "ecmo",
    "epidemic",
    "epitwitter",
    "essentialworkers",
    "face masks",
    "face shield",
    "facemask",
    "facemasks",
    "ffp1",
    "ffp2",
    "ffp2 mask",
    "ffp3",
    "ffp3 mask",
    "fightagainstcorona",
    "fightagainstcoronavirus",
    "fightcoronatogether",
    "fightcovid19",
    "fighttogether",
    "flatten the curve",
    "flattenthecurve",
    "floridacoronavirus",
    "frontline health care workers",
    "getmeppe",
    "gilead",
    "goggle",
    "hackthepandemic",
    "hand - washing",
    "health workers",
    "healthcare",
    "healthcareheroes",
    "healthheroes",
    "healthworkers",
    "healthyathome",
    "homequarantine",
    "hydroxycloroquine",
    "immune system",
    "indiafightscorona",
    "indiafightscoronavirus",
    "indiafightscovid19",
    "infection",
    "infections",
    "injectdisenfectant",
    "inmyquarantinesurvivalkit",
    "inthistogether",
    "istayhomeforyou",
    "kn95",
    "kn95 mask",
    "koronavirus",
    "kungflu",
    "lab4life",
    "labheroes",
    "lablife",
    "labweek",
    "labweek2020",
    "level4lockdown",
    "lock down",
    "lockdown",
    "lockdownextension",
    "lockdownindia",
    "lockdownzim",
    "mask",
    "masks4all",
    "n - 95 respirator masks",
    "n95",
    "n95 mask",
    "n95 respirator mask",
    "n95 / kn95 respirator",
    "ncov",
    "nhscovidheroes",
    "no shaking hands",
    "nucleic acid",
    "nucleic acid test",
    "nucleic acid test kit",
    "nucleic acid tests",
    "nucleic - acid based test kits",
    "nursescovid19",
    "nypause",
    "outbreak",
    "pandamic",
    "pandemia",
    "pandemic",
    "pandemic2020",
    "pandemie",
    "panic buy",
    "panic shop",
    "panic shopping",
    "panic - buy",
    "panic - shop",
    "panicbuy",
    "panicbuyingpanic buying",
    "panicshop",
    "personalprotectiveequipment",
    "pharma",
    "physicaldistancing",
    "plankthecurve",
    "plasma",
    "ppe",
    "ppeshortage",
    "prayerforhealing",
    "quarantine",
    "quarantine shelter",
    "quarantinelife",
    "quarantineworkout",
    "quarentinelife",
    "remdesivir",
    "respirator",
    "safehands",
    "saferathome",
    "sanitizer",
    "sars - cov - 2",
    "sars - cov - 2 virus",
    "sarscov2",
    "savelives",
    "saynobillgatesvaccine",
    "sflockdown",
    "sheltering in place",
    "shelteringinplace",
    "sinophobia",
    "slow the spread of covid19",
    "social distance",
    "social distancing",
    "socialdistance",
    "socialdistancing",
    "socialdistancingnow",
    "standtogether",
    "stay at home",
    "stay home",
    "stay home challenge",
    "stay safe stay home",
    "stay_home_safe",
    "stayathome",
    "stayathomeandstaysafe",
    "stayathomesa",
    "stayathomesavelives",
    "stayhealthy",
    "stayhome",
    "stayhome!",
    "stayhomechallenge",
    "stayhomesavelives",
    "stayhomestaysafe",
    "stayhometosavelives",
    "stayinghome",
    "stayingsafe",
    "stayinstaysafe",
    "staysafe",
    "staysafestayhealthy",
    "staysafestayhome",
    "stopthespread",
    "strongertogether",
    "supportingnursesandmidwives",
    "supportnursesandmidwives",
    "supportsmallbusinesses",
    "surgical",
    "gloves",
    "surgical",
    "gwon",
    "surgical",
    "mask",
    "surgical",
    "nose",
    "mask",
    "symptoms",
    "tedros",
    "adhanom",
    "ghebreyesus",
    "thankahealthcarehero",
    "thankshealthheroes",
    "thankyoucoronawarriors",
    "thankyouessentialworkers",
    "thankyouhealthcareworkers",
    "togetherathome",
    "togetherwecan",
    "togetherwecandoit",
    "treat",
    "coronavirus",
    "trump",
    "pandemic",
    "trumpdisinfectant",
    "trumppandemic",
    "uniteagainstcovid19",
    "vaccination",
    "vaccine",
    "vaccine",
    "against",
    "coronavirus",
    "vaccineswork",
    "ventilator",
    "videoconferencing",
    "wash",
    "your",
    "hands",
    "washyourhands",
    "wear",
    "face",
    "mask",
    "wearmask",
    "wearmasks",
    "wehealasone",
    "who",
    "director - general",
    "workfromhome",
    "world",
    "health",
    "organization(who)",
    "wuhan",
    "wuhancoronavirus",
    "wuhanlockdown",
    "yearofnurses",
    "zimlockdown"
]


def search_for_tweets(terms):
    for item in terms:
        print("item", item)
        # configuration
        config = twint.Config()
        config.Search = str(item)
        config.Custom["tweet"] = [
            "id",
            "conversation_id",
            "created_at",
            "date",
            "time",
            "timezone",
            "user_id",
            "username",
            "name",
            "place",
            "tweet",
            "language",
            "mentions",
            "urls",
            "photos",
            "replies_count",
            "retweets_count",
            "likes_count",
            "hashtags",
            "cashtags",
            "link",
            "retweet",
            "quote_url",
            "video",
            "thumbnail",
            "near",
            "geo",
            "user_rt_id",
            "user_rt",
            "source",
            "retweet_date",
            "translate",
            "trans_src",
            "trans_dest"
        ]
        config.Output = str(item) + ".csv"
        config.Store_csv = True
        config.Since = "2020-03-15"
        config.Until = "2020-03-16"
        config.Lang = "en"
        config.Tabs = True

        # running search
        twint.run.Search(config)


if __name__ == "__main__":
    search_for_tweets(["corona"])
